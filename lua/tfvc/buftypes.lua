...
---@param buf number
local function get_changeset_from_line(buf)
  local mode = vim.api.nvim_get_mode().mode
  if mode == 'v' or mode == 'V' then

    -- mixed 0 and 1 based indexing is fucking hell...
    -- this returns 1-based linenumbers (like what is shown in the editor as linenumber)
    --
    -- also: ...
    -- apparently the most reliable way to get the start and end lines of the current visual selection
    -- vim.api.nvim_buf_get_mark(_, '<') doesn't update until visual mode exists
    local s = vim.fn.getpos('v')[2]
    local e = vim.fn.getpos('.')[2]

    -- this takes 0-based line numbers (hence range from x-1, to x)
  ...
  callback = function(args)
    ...
    u.tf_cmd(cmd, tfcmdOpts, vim.schedule_wrap(function(obj)
      ...
      local function open_cs()
        local cs_number = get_changeset_from_line(buf)
        if cs_number then
      ...
      if fsinfo.type == 'file' then
        ...
        vim.keymap.set('v', 'd', function ()
          local cs_start, cs_end = get_changeset_from_line(buf)
