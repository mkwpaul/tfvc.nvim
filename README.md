...
verbose output for debugging. You should probably leave this unset.
Default is false.

- `default_versionspec`

Versionspec to use with commands when no versionspec is
specified, Default is 'T' which indicates to use the latest server version.

- `diff_no_split`

if true, then hide the buffer that is compared against, when
...
When using TF status, only show changed files under the
current working directory Default is true, i.e. do filter by CWD.

- `history_entry_limit`

Number of entries to load in history buffers.
Default is 300 entries.
...
if specified, use iconv to convert output from tf.exe from
